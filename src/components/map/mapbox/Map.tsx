...
const ReactMap = ({
  ...
  const context = useContext(SearchResultLayoutContext);
  let isShowingResult = null;
  if (context) {
    isShowingResult = context.isShowingResult;
  }

  // to fix an existing issue in mapbox: when the width of map's parent component changed,
  // the canvas generated by the map doesn't resize
  useEffect(() => {
    map?.resize();
  }, [isShowingResult, map]);
