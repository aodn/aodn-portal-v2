...
def handle_detail_summary_api(route: Route) -> None:
    ...
    print(f"[MOCK API] handle_detail_summary_api called for URL: {route.request.url}, data_id: {data_id}")
    try:
        json_data = load_json_data(f'summary/{data_id}.json')
        print(f"[MOCK API] Successfully loaded summary/{data_id}.json. Features count: {len(json_data.get('features', []))}")
        route.fulfill(json=json_data)
    except FileNotFoundError:
        print(f"[MOCK API] File summary/{data_id}.json NOT found. Returning empty dict.")
